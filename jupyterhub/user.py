...
class User:
    ...
    async def sync_roles(self, auth_roles):
        ...
        predef_managed_roles = await self.authenticator.load_managed_roles()
        predef_managed_role_names = [r.get('name') for r in predef_managed_roles]
        ...
        for stripped_role in managed_stripped_roles:
            if (
                ...
                and not (self.authenticator.reset_managed_roles_on_startup and stripped_role.name in predef_managed_role_names)
            ):
